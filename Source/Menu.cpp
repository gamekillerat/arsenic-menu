...
int Armor_ = 0, Brakes_ = 0, Transmission_ = 0, Bumpers_ = 0, Hoods_ = 0, Suspension_ = 0, Spoilers_ = 0, Skirts_ = 0,
Exhaust_ = 0, Grill_ = 0, Interior_ = 0, Tint_ = 0, horns = 0, livery = 0;

bool wallDrive = 0, Rainb = 0, Drift = 0, Gravity = 0, forceog = 0, Rneon = 0, ShootR = 0, NameESP = 0, Trigger = 0, Superr = 0,
UltraJ = 0, ShootT = 0, ShootTankR = 0, ShootBall = 0, ShootMini = 0, ShootWeaponT = 0, ShootHydra = 0, ShootDump = 0, ShootBuz = 0;
bool vehjump = 0, freezespawned = 0, numbersChanged = 0, playerGodMode = 0, para = 0, savenewdrop21 = 0; bool invis = 0; float MoveX = 0.5f; float MoveZ = 0.5f; int Face_ = 0; int tpgun = 0;
int timeer = 0; long in = 0; int intor = 0; long out = 0; bool Invisablecar = 0; bool bulletTime = 0; bool loop_SuperGrip = 0; bool SuperGrip = 0, Analogo = 0; bool stream = false; bool risky;
...
char* buildingobjects[] = {
	"cs6_01_motel_rm2","cs6_03_radio_sta","cs6_10_desert_hse_2a","cs6_10_deserthouse_007","cs6_10_build2","cs6_03_house02",
	"cs6_06_jstorage","cs6_06_jhouse","cs6_06_farmhouse","cs6_08_house1","cs6_08_church",
	"cs6_08a_struct06","cs6_10_building_farm","cs6_10_gas_station",
};
char* landobjects[] = {
	"cs6_08_land_01","cs6_08a_land_09","cs6_08a_land_08","cs6_08a_land_07","cs6_08a_land_06",
	"cs6_08a_land_05","cs6_08a_land_03","cs6_08a_land_01","cs6_09_land_09","cs6_09_land_05",
	"cs6_09_land_04","cs6_09_land_03","cs6_10_land03"
};
char* pedModels[] = {
...
bool Features::freezeobject;
void spawn_object(std::string yh)
{
	Vector3 pos = ENTITY::GET_ENTITY_COORDS(PLAYER::PLAYER_PED_ID(), 1);
	Object custom = SpawnObject((char*)_strdup(yh.c_str()), pos);
	if (custom != -1)
	{
		latestObj = custom;
		Vector3 objCoords = ENTITY::GET_ENTITY_COORDS(latestObj, 1);
		Vector3 objRot = ENTITY::GET_ENTITY_ROTATION(latestObj, 0);
		if (Features::freezeobject)
		{
			ENTITY::FREEZE_ENTITY_POSITION(latestObj, true);
		}
	}
}
...
void wait() {
	...
	void main() {
		UI::_SET_NOTIFICATION_TEXT_ENTRY("STRING");
		UI::_ADD_TEXT_COMPONENT_STRING("Welcome back ars3n1c, you are using VIP | Release 1.1.2 beta");
		UI::_SET_NOTIFICATION_MESSAGE_CLAN_TAG_2("CHAR_SOCIAL_CLUB", "CHAR_SOCIAL_CLUB", 1, 8, "~r~ars3n1c", "~c~v1.2.2 beta", 1, "___ARSN",9);
		...
		case mainmenu:
		{
			...
			Menu::MenuOption("Object Editor", modelObjects);
		...
		case teleport:
		{
			...
			if (Menu::Option("Teleport To Waypoint")) {
				...
				if (UI::DOES_BLIP_EXIST(WaypointHandle))
				{
					...
					{
						ENTITY::SET_ENTITY_COORDS(Handle, WaypointPos.x, WaypointPos.y, groundCheckHeight[i], 0, 0, 0, 1);
						WAIT(100);
						if (GAMEPLAY::GET_GROUND_Z_FOR_3D_COORD(WaypointPos.x, WaypointPos.y, groundCheckHeight[i], &WaypointPos.z, 0))
						{
							groundFound = true;
							WaypointPos.z += 1.0;
							break;
		...
		case onlinemenu_selected:
		{
			...
			if (Menu::Option("~r~Host ~w~Kick Player")) {
			...
			if (Menu::Option("Spawn Bodyguard")) {
				...
				Hash railgun = GAMEPLAY::GET_HASH_KEY("WEAPON_SPECIALCARBINE_MK2");
				Vector3 pos = ENTITY::GET_ENTITY_COORDS(selectedplayer, 1);
				Hash pedm = GAMEPLAY::GET_HASH_KEY("S_M_M_Security_01");
			...
			if (Features::Online::selectedPlayer != PLAYER::PLAYER_ID()) { Menu::Toggle("Spectate Player", Features::spectate[Features::Online::selectedPlayer], [] { Features::specter(Features::spectate[Features::Online::selectedPlayer]); }); }
			...
			Menu::Toggle("Money Drop", Features::savenewdrop2[Features::Online::selectedPlayer], [] {Features::dildmoney(Features::savenewdrop2[Features::Online::selectedPlayer]); });
		...
		{
			Menu::Title("");
			Menu::Subtitle("Attach To Player");
			...
			if (Menu::Option("~r~Delete All Objects")) {
				for (int i = 0; i < 5; i++) {
					...
					for (int i = 0; i < 136; i++) {
						Object obj = OBJECT::GET_CLOSEST_OBJECT_OF_TYPE(coords.x, coords.y, coords.z, 4.0, GAMEPLAY::GET_HASH_KEY(objectModels[i]), 0, 0, 1);
		...
		case allplayers:
		{
			...
			Menu::MenuOption("ESP Menu", espmenu);
			Menu::MenuOption("Weapon Menu", onlinegive);
			Menu::MenuOption("World Options", worldoptions);
			Menu::MenuOption("Money Drop Place", moneydropplace);
			if (Menu::Option("Show Host")) {
				for (int i = 0; i <= 32; i++)
				{
					WAIT(0);
			...
			Menu::Toggle("Always Wanted", Features::everyonealwayswanted);
		...
		case onlinegive:
			Menu::Title("");
			Menu::Subtitle("Online Player Weapons");
		if (Menu::Option("Give All Weapons")) {
			static LPCSTR weaponNames2[] = {
				...
				"WEAPON_MARKSMANRIFLE", "WEAPON_HEAVYSHOTGUN", "WEAPON_GUSENBERG", "WEAPON_HATCHET", "WEAPON_RAILGUN", "WEAPON_FLASHLIGHT", "WEAPON_MACHINEPISTOL", "WEAPON_MACHETE","WEAPON_PISTOL_MK2",
				"WEAPON_SMG_MK2","WEAPON_ASSAULTRIFLE_MK2","WEAPON_CARBINERIFLE_MK2","WEAPON_COMBATMG_MK2","WEAPON_HEAVYSNIPER_MK2"
			};
			{
				for (int i = 0; i <= 32; i++)
				{
					...
					WAIT(100);
					{
						...
						notifyMap("~g~Weapons Given To Everyone :)");
		...
		if (Menu::Option("Give them Railguns")) {
			for (int i = 0; i <= 32; i++)
			{
				...
				WEAPON::GIVE_WEAPON_TO_PED(Handle, railgun, railgun, 9999, 9999);
				{
					...
					notifyMap("~g~Everyone Has A Railgun Now, Look Out");

				}
			}
		}
		if (Menu::Option("Give them StunGun")) {
			for (int i = 0; i <= 32; i++)
			{
				WAIT(0);
				if (i == PLAYER::PLAYER_ID())continue;
				...
				WEAPON::GIVE_WEAPON_TO_PED(Handle, railgun, railgun, 9999, 9999);
				{
					...
					notifyMap("~g~Everyone Has A Stungun Now, Look Out");
		...
		{
			Menu::Title("");
			Menu::Subtitle("ESP Options");
			Menu::Toggle("Show Name Tags", Features::NameESP);
			Menu::Toggle("ESP", Features::esper);
		}
		break;
#pragma endregion
#pragma region Money_drop_PLace
		case moneydropplace:
			Menu::Title("");
			Menu::Subtitle("Money Drop Locations");
			Menu::Toggle("AirPort", Features::ConeDrop);
			if (Menu::Option("Teleport"))
			{
				Vector3 cone;
				cone.x = -1469.539; cone.y = -2730.460; cone.z = 13.945;
				TPto(cone);
			}
			Menu::Toggle("Chillard", Features::chillarddrop);
			if (Menu::Option("Teleport"))
			{
				Vector3 drop;
				drop.x = 492.108; drop.y = 5589.875; drop.z = 794.038;
				TPto(drop);
			}
			Menu::Toggle("Maze Bank", Features::mazedrop);
			if (Menu::Option("Teleport"))
			{
				Vector3 cone;
				cone.x = -74.759; cone.y = -818.727; cone.z = 326.149;
				TPto(cone);
			}
			break;
		case online:
		{
			Menu::Title("");
			Menu::Subtitle("General Online");
			Menu::MenuOption("Recovery", recovery);
			Menu::MenuOption("Protections", protectedmenu);
			Menu::Break("In Development");
			Menu::Toggle("No Orbital Cannon Cooldown", Features::unlimitedorbitalcannon);
			Menu::Toggle("Off Radar", Features::orbool);
		}
		break;
		case protectedmenu:
		{
			Menu::Title("");
			Menu::Subtitle("Protection Menu");
			Menu::Toggle("No More Objects", Features::DeleteObjectLoop);
			Menu::Toggle("Clear Task", Features::blockcleartask);
			Menu::Toggle("No More Invites", Features::NoInvites);
			Menu::Toggle("Kick Votes Remote", Features::votes);
			Menu::Toggle("Explosions Remote", Features::explosion);
			Menu::Toggle("Fire Remote", Features::profire);
			Menu::Toggle("Remove Weapon Remote", Features::revweapons);
			Menu::Toggle("Remote", Features::remote);
			Menu::Toggle("Report Attacks Remote", Features::report);
			Menu::Toggle("Attacks Remote", Features::att);
			Menu::Toggle("Weather Remote", Features::WEATHER);
			Menu::Toggle("Wanted Remote", Features::WANTED);
			Menu::Toggle("Wanted Level Remote", Features::WANTED_LEVEL);
			//Menu::Toggle("Ragdoll Remote", Features::RAGDOLL, [] { Features::REQUESTEVENT(); });
			Menu::Toggle("Player Taunt Remote", Features::TAUNT);
			Menu::Toggle("Network Remote", Features::NETWORK);
			Menu::Toggle("Network CRC Remote", Features::NETWORKCRC);
			//Menu::Toggle("Money Remote", Features::bypass1, [] { Features::moneyBypass1(); });
		}
		break;
		case recovery:
		{
			...
			Menu::MenuOption("Unlocks", unlocks);
			Menu::MenuOption("Rank", level1);
			if (Menu::Option("15 Mil Stealth"))
			{
			...
			if (Menu::Option("10 Mil Stealth"))
			{

				Any var0 = 0ul;
				UNK3::_NETWORK_SHOP_BEGIN_SERVICE(&var0, 1474183246, 312105838, 1445302971, 10000000, 4);
				UNK3::_NETWORK_SHOP_CHECKOUT_START(var0);
		...
		case unlocks:
		{
			...
			if (Menu::Option("Chrome Rims"))
			{
				...
				UI::_SET_NOTIFICATION_TEXT_ENTRY("STRING");
				UI::ADD_TEXT_COMPONENT_SUBSTRING_PLAYER_NAME("Free Chrome Rim Unlocked");
				UI::_SET_NOTIFICATION_MESSAGE_CLAN_TAG_2("CHAR_SOCIAL_CLUB", "CHAR_SOCIAL_CLUB", true, 8, "ars3n1c", "", 1, "ARSN", 9);
				UI::_DRAW_NOTIFICATION(FALSE, FALSE);
			}
			if (Menu::Option("Max Skills")) {
				...
				UI::_SET_NOTIFICATION_TEXT_ENTRY("STRING");
				UI::ADD_TEXT_COMPONENT_SUBSTRING_PLAYER_NAME("Skills Maxed");
				UI::_SET_NOTIFICATION_MESSAGE_CLAN_TAG_2("CHAR_SOCIAL_CLUB", "CHAR_SOCIAL_CLUB", true, 8, "ars3n1c", "", 1, "ARSN", 9);
				UI::_DRAW_NOTIFICATION(FALSE, FALSE);
			}
			if (Menu::Option("Clear Reports")) {
				...
				STATS::STAT_SET_INT(GAMEPLAY::GET_HASH_KEY("MP1_MPPLY_GAME_EXPLOITS"), 0, 1);
				STATS::STAT_SET_INT(GAMEPLAY::GET_HASH_KEY("MP1_MPPLY_ISPUNISHED"), 0, 1);
				STATS::STAT_SET_FLOAT(GAMEPLAY::GET_HASH_KEY("MP0_MPPLY_OVERALL_BADSPORT"), 0, 1);
				STATS::STAT_SET_BOOL(GAMEPLAY::GET_HASH_KEY("MP0_MPPLY_CHAR_IS_BADSPORT"), false, 1);
				...
				UI::_SET_NOTIFICATION_TEXT_ENTRY("STRING");
				UI::ADD_TEXT_COMPONENT_SUBSTRING_PLAYER_NAME("Reports Cleared");
				UI::_SET_NOTIFICATION_MESSAGE_CLAN_TAG_2("CHAR_SOCIAL_CLUB", "CHAR_SOCIAL_CLUB", true, 8, "ars3n1c", "", 1, "ARSN", 9);
				UI::_DRAW_NOTIFICATION(FALSE, FALSE);
			}
			if (Menu::Option("Unlock All Tattoos")) {
				...
				UI::_SET_NOTIFICATION_TEXT_ENTRY("STRING");
				UI::ADD_TEXT_COMPONENT_SUBSTRING_PLAYER_NAME("All tattoos unlocked");
				UI::_SET_NOTIFICATION_MESSAGE_CLAN_TAG_2("CHAR_SOCIAL_CLUB", "CHAR_SOCIAL_CLUB", true, 8, "ars3n1c", "", 1, "ARSN", 9);
				UI::_DRAW_NOTIFICATION(FALSE, FALSE);
			}
			if (Menu::Option("Add Snacks")) {
				...
				UI::_SET_NOTIFICATION_TEXT_ENTRY("STRING");
				UI::ADD_TEXT_COMPONENT_SUBSTRING_PLAYER_NAME("All Snacks Given");
				UI::_SET_NOTIFICATION_MESSAGE_CLAN_TAG_2("CHAR_SOCIAL_CLUB", "CHAR_SOCIAL_CLUB", true, 8, "ars3n1c", "", 1, "ARSN", 9);
				UI::_DRAW_NOTIFICATION(FALSE, FALSE);
			}
			if (Menu::Option("Unlock Weapon Liveries")) {
				...
				UI::_SET_NOTIFICATION_TEXT_ENTRY("STRING");
				UI::ADD_TEXT_COMPONENT_SUBSTRING_PLAYER_NAME("Weapon Liveries Unlocked");
				UI::_SET_NOTIFICATION_MESSAGE_CLAN_TAG_2("CHAR_SOCIAL_CLUB", "CHAR_SOCIAL_CLUB", true, 8, "ars3n1c", "", 1, "ARSN", 9);
				UI::_DRAW_NOTIFICATION(FALSE, FALSE);
			}
			if (Menu::Option("Purchase Clothing")) {
				...
				STATS::STAT_SET_INT(GAMEPLAY::GET_HASH_KEY("MP0_ADMIN_CLOTHES_GV_BS_12"), -1, 1);
				UI::_SET_NOTIFICATION_TEXT_ENTRY("STRING");
				UI::ADD_TEXT_COMPONENT_SUBSTRING_PLAYER_NAME("All Clothing Purchased");
				UI::_SET_NOTIFICATION_MESSAGE_CLAN_TAG_2("CHAR_SOCIAL_CLUB", "CHAR_SOCIAL_CLUB", true, 8, "ars3n1c", "", 1, "ARSN", 9);
				UI::_DRAW_NOTIFICATION(FALSE, FALSE);
			}
			if (Menu::Option("Full Recovery")) {
				STATS::STAT_SET_INT(GAMEPLAY::GET_HASH_KEY("MP0_CHAR_XP_FM"), Levels[120], 0);
				STATS::STAT_SET_INT(GAMEPLAY::GET_HASH_KEY("MP0_SCRIPT_INCREASE_STAM"), 100, 0);
				STATS::STAT_SET_INT(GAMEPLAY::GET_HASH_KEY("MP0_SCRIPT_INCREASE_STRN"), 100, 0);
				STATS::STAT_SET_INT(GAMEPLAY::GET_HASH_KEY("MP0_SCRIPT_INCREASE_LUNG"), 100, 0);
				STATS::STAT_SET_INT(GAMEPLAY::GET_HASH_KEY("MP0_SCRIPT_INCREASE_DRIV"), 100, 0);
				...
				STATS::STAT_SET_INT(GAMEPLAY::GET_HASH_KEY("MP0_ADMIN_CLOTHES_GV_BS_12"), -1, 1);
				UI::_SET_NOTIFICATION_TEXT_ENTRY("STRING");
				UI::ADD_TEXT_COMPONENT_SUBSTRING_PLAYER_NAME("~g~Recovery Completed");
				UI::_SET_NOTIFICATION_MESSAGE_CLAN_TAG_2("CHAR_SOCIAL_CLUB", "CHAR_SOCIAL_CLUB", true, 8, "ars3n1c", "", 1, "ARSN", 9);
				UI::_DRAW_NOTIFICATION(FALSE, FALSE);
			}
			if (Menu::Option("Unlock LSC")) {
				...
				STATS::STAT_SET_INT(GAMEPLAY::GET_HASH_KEY("MP0_AWD_20_KILLS_MELEE"), 50, 0);
				UI::_SET_NOTIFICATION_TEXT_ENTRY("STRING");
				UI::ADD_TEXT_COMPONENT_SUBSTRING_PLAYER_NAME("Vehicle Options Unlocked");
				UI::_SET_NOTIFICATION_MESSAGE_CLAN_TAG_2("CHAR_SOCIAL_CLUB", "CHAR_SOCIAL_CLUB", true, 8, "ars3n1c", "", 1, "ARSN", 9);
				UI::_DRAW_NOTIFICATION(FALSE, FALSE);
			}
		}
		...
		case self:
		{
			...
			Menu::MenuOption("Wardrobe", OutfitOptions);
			Menu::MenuOption("Model Changer", modelchanger);
			Menu::MenuOption("Effects", PTFX);
			Menu::MenuOption("Ridable Animals", ride);
			Menu::MenuOption("Pets", pet);
			Menu::MenuOption("Vision", visionmenu);
			...
			if (Menu::Option("BST")) { globalHandle(2436641).At(3875).As<int>() = NETWORK::GET_NETWORK_TIME(); };
			Menu::Toggle("Super Run", Features::SonicRun);
			Menu::Toggle("Super Swim", Features::fastswim);
			Menu::Float("Run Style", Features::customRun, 0, 25);
			Menu::Toggle("Activate", Features::customrun);
			Menu::Toggle("SuperMan Mode", Features::Superr);
			//Menu::Int("Player Height", Features::playerheight, 0, 250, 10);
			Menu::Option("Clean Ped", [] { int Ped = PLAYER::PLAYER_PED_ID(); PED::CLEAR_PED_BLOOD_DAMAGE(Ped); PED::RESET_PED_VISIBLE_DAMAGE(Ped); });
			Menu::Option("Max Health", [] {int Ped = PLAYER::PLAYER_PED_ID(); PED::SET_PED_MAX_HEALTH(Ped, 100); });
			Menu::Option("Max Armour", [] {int Ped = PLAYER::PLAYER_PED_ID(); PED::ADD_ARMOUR_TO_PED(Ped, 100); });
			Menu::Toggle("Slow Mo", Features::SlowMo);
			Menu::Toggle("No-Collision", Features::pedcoll);
			Menu::Toggle("Drunk", Features::drunk);
		...
		case visionmenu:
			Menu::Title("");
			Menu::Subtitle("Vision Menu");
			Menu::Toggle("Night Vision", Features::nightvision, [] {Features::nightvisionvoid(Features::nightvision); });
			Menu::Toggle("Thermal Vision", Features::thermalvision, [] {Features::visionstuff(Features::thermalvision); });
			if (Menu::Option("Default")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("DEFAULT"); }
			if (Menu::Option("Drunk")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("Drunk"); }
			if (Menu::Option("Stoned")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("stoned"); }
			if (Menu::Option("Orange")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("REDMIST"); }
			if (Menu::Option("Death")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("DEATH"); }
			if (Menu::Option("Drugs")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("drug_flying_base"); }
			if (Menu::Option("Huffin Gas")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("DRUG_gas_huffin"); }
			if (Menu::Option("Tipsy")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("drug_wobbly"); }
			if (Menu::Option("Heaven")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("Bloom"); }
			if (Menu::Option("Fancy")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("PlayerSwitchPulse"); }
			if (Menu::Option("Hallucinations")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("player_transition"); }
			if (Menu::Option("Killstreak")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("MP_Killstreak"); }
			if (Menu::Option("Low Quality")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("cinema_001"); }
			if (Menu::Option("Blurry")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("CHOP"); }
			if (Menu::Option("Blinding")) { GRAPHICS::SET_TIMECYCLE_MODIFIER("BarryFadeOut"); }

			break;
		case PTFX:
		{
			Menu::Title("");
			Menu::Subtitle("Player Effects");
		...
		case senas:
		{
			...
			if (Menu::Option("~r~Clear ~w~Animation")) AI::CLEAR_PED_TASKS_IMMEDIATELY(PLAYER::PLAYER_PED_ID());
...
#pragma region Outfit Creator
		...
		case vehicle:
		{
			...
			Menu::Toggle("Drift Mode", Features::driftmode, [] {Features::drifter(Features::driftmode); });
		...
		case vehspawner:
		{
			//give me
			Menu::Title("");
			Menu::Subtitle("Vehicle Spawner Menu");
			Menu::MenuOption("Spawner Settings", spawnsettings);
			Menu::MenuOption("Southern San Andreas", san);
			Menu::MenuOption("Smugglers Run", SmuglerRun1);
			Menu::MenuOption("Gun Running", GunRunning1);
			Menu::MenuOption("Cunning Stunts", CunningStunts1);
			Menu::MenuOption("Import/Export", ImportEx1);
			...
			if (Menu::Option("Input Vehicle"))
			{
				...
				if (STREAMING::IS_MODEL_VALID(model))
				{
					...
					if (Features::spawnmaxed)
					{
						...
						VEHICLE::SET_VEHICLE_NUMBER_PLATE_TEXT(veh, "ars3n1c");
					}
					WAIT(150);
					STREAMING::SET_MODEL_AS_NO_LONGER_NEEDED(model);
				}
		...
		case spawnsettings:
			Menu::Title("");
			Menu::Subtitle("Spawn Settings");
			Menu::Toggle("Spawn in Car", Features::spawnincar);
			Menu::Toggle("Spawn Maxed", Features::spawnmaxed);
			Menu::MenuOption("Custom Colour", spawncustomcolour);
			//Menu::Toggle("Spawn With Blip", Features::spawnwithblip);
			break;
#pragma endregion
#pragma region set custom colour
		case spawncustomcolour:
			Menu::Title("");
			Menu::Subtitle("Set Custom Colour Menu");
			Menu::Break("Primary Colour");
			Menu::Int("Red: ", Features::pricustomred, 0, 255);
			Menu::Int("Green: ", Features::pricustomgreen, 0, 255);
			Menu::Int("Blue: ", Features::pricustomblue, 0, 255);
			Menu::Toggle("Set", Features::setcustompricolour);
			Menu::Break("Secondary Colour");
			Menu::Int("Red: ", Features::seccustomred, 0, 255);
			Menu::Int("Green: ", Features::seccustomgreen, 0, 255);
			Menu::Int("Blue: ", Features::seccustomblue, 0, 255);
			Menu::Toggle("Set", Features::setcustomseccolour);
			break;
		case ImportEx1:
			Menu::Title("");
			Menu::Subtitle("Import/Export");
			for (auto car : IMPORTEXPORT1) { Menu::Option(car, [car] { Features::spawn_vehicle(car); }); }
			break;
		case CunningStunts1:
			Menu::Title("");
			Menu::Subtitle("Cunning Stunts");
			for (auto car : CUNNINGSTUNTS1) { Menu::Option(car, [car] { Features::spawn_vehicle(car); }); }
			break;
		case GunRunning1:
			Menu::Title("");
			Menu::Subtitle("Gun Running");
			for (auto car : SMUGLERSRUN1) { Menu::Option(car, [car] { Features::spawn_vehicle(car); }); }
			break;
		case SmuglerRun1:
			Menu::Title("Smuglers Run");
			Menu::Subtitle("Smuglers Run");
			for (auto car : GUNRUNNING1) { Menu::Option(car, [car] { Features::spawn_vehicle(car); }); }
			break;
		case Doomsday:
			Menu::Title("");
			Menu::Subtitle("Doomsday");
			for (auto car : Doomsday1) { Menu::Option(car, [car] { Features::spawn_vehicle(car); }); }
			break;
		case san:
			Menu::Title("");
			Menu::Subtitle("Southern San");
			for (auto car : SOUTHERNSAN) { Menu::Option(car, [car] { Features::spawn_vehicle(car); }); }
			break;
		case Supers:
			Menu::Title("");
			Menu::Subtitle("Super");
		...
		case trucks:
			Menu::Title("");
			Menu::Subtitle("Trucks");
			for (auto car : trucks1) { Menu::Option(car, [car] {Features::spawn_vehicle(car); }); }
			break;
		}
		break;
		case plateoptions:
		...
		case lsc:
		{
			...
			Menu::Int("Livery", livery, 0, 25); { int VehID = PED::GET_VEHICLE_PED_IS_IN(PLAYER::PLAYER_PED_ID(), 0);
			VEHICLE::SET_VEHICLE_MOD_KIT(VehID, livery);
			VEHICLE::SET_VEHICLE_MOD(VehID, 25, livery, 0);
			}
		...
		{//weapon menu 
			Menu::Title("");
			Menu::Subtitle("Weapon Menu");
			//Menu::MenuOption("Shoot Entity", shootentity);
			Menu::MenuOption("Projectile", projectile);
			if (Menu::Option("Give All Weapons")) {
			...
			/*if (Menu::Option("Max Components"))
			{
			uint WeaponComonents[] = { 0xE7939662,0x5B3E7DB6,0x9137A500,0x9137A500,0xE28BABEF,0x3F4E8AA6,0x3F4E8AA6,0x9761D9DC,0x50910C31 ,0xC613F685 ,0xF3462F33 ,0x9493B80D ,0x16EE3040 ,0x937ED0B7 ,0x84C8B2D3 ,0x9BC64089 ,0x161E9241 ,0x7A6A7B7B ,0x77B8AB2F ,0x4032B5E7 ,0x4EAD7533 ,0x4EAD7533 ,0x377CD377 ,0x85A64DF9 ,0x487AAE09 ,0x9B76C72C ,0xD7391086 ,0x27872C90 ,0xE608B35E ,0xC304849A ,0xA73D4664 ,0x837445AA ,0x65EA7EBB ,0x3C00AFED ,0x3CC6BA57 ,0xD2443DDC ,0xAA2C45B4 ,0xA0D89C42 ,0x9D2FBF29 ,0xE1FFB34A ,0xD4A969A ,0xC6C7E581 ,0xC5A12F80 ,0xC164F53 ,0x7BC4CDDC ,0x359B7AAE ,0x971CF6FD ,0x324F2D5F ,0xCCFD2AC5 ,0x91109691 ,0xB1214F9B ,0xEAC8C270 ,0x1CE5A6A5 ,0x82158B47 ,0x334A5203 ,0xBB46E417 ,0x350966FB,0x10E6BA2B,0x33BA12E8,0xD9D3AC92,0xED265A1C,0xD6C59CD6,0xB3688B0F ,0x8EC1C979 ,0x86BD7F72 ,0x7C8BD10E ,0x8033ECAF ,0x7B0033B3 ,0x64F9C62B ,0x249A17D5 ,0x249A17D5 ,0x59FF9BF8 ,0xB92C6979 ,0x108AB09E ,0x108AB09E ,0x2E43DA41 ,0xB5E2575B ,0x17DF42E9 ,0x8B3C480B ,0x5DD5DBD5 ,0x5646C26A ,0xC66B6542 , 0xC66B6542 ,0xD12ACA6F ,0x420FD713 ,0xA564D78B ,0xE502AB6B ,0xB9835B2E ,0x21E34793 ,0x43FD595B ,0x5ED6C128 };
			uint Weapons[] = { 0x99B507EA, 0x678B81B1, 0x4E875F73, 0x958A4A8F, 0x440E4788, 0x84BD7BFD, 0x1B06D571, 0x5EF9FEC4, 0x22D8FE39, 0x99AEEB3B, 0x13532244, 0x2BE6766B, 0xEFE7E2DF, 0xBFEFFF6D, 0x83BF0278, 0xAF113F99, 0x9D07F764, 0x7FD62962, 0x1D073A89, 0x7846A318, 0xE284C527, 0x9D61E50F, 0x3656C8C1, 0x05FC3C11, 0x0C472FE2, 0x33058E22, 0xA284510B, 0x4DD2DC56, 0xB1CA77B1, 0x687652CE, 0x42BF8A85, 0x93E220BD, 0x2C3731D9, 0xFDBC8A50, 0x24B17070, 0x060EC506, 0x34A67B97, 0xFDBADCED, 0x23C9F95C, 0x497FACC3, 0xF9E6AA4B, 0x61012683, 0xC0A3098D, 0xD205520E, 0xBFD21232, 0x7F229F94, 0x92A27487, 0x083839C4, 0x7F7497E5, 0xA89CB99E, 0x3AABBBAA, 0xC734385A, 0x787F0BB, 0x47757124, 0xD04C944D };
			for (int i = 0; i < sizeof(Weapons) / sizeof(Weapons[0]); i++)
			for (int s = 0; s < sizeof(WeaponComonents) / sizeof(WeaponComonents[0]); s++)
			WEAPON::GIVE_WEAPON_COMPONENT_TO_PED(PLAYER::PLAYER_PED_ID(), GAMEPLAY::GET_HASH_KEY((char *)Weapons[i]), GAMEPLAY::GET_HASH_KEY((char *)WeaponComonents[s]));
			break;
			}*/
			//Menu::Toggle("Amming Lazer", Features::weaplazer, [] {Features::WeapLazer(Features::weaplazer); });
			...
			//Menu::Toggle("Explosive Ammo", Features::explosiveammo, [] {Features::expammo(Features::explosiveammo); });
			//Menu::ToggleInt("Custom Explosive Ammo", Features::Customexpl, Features::customex, 0, 35);
			...
			//Menu::Toggle("Gravity Gun", Features::Gravity);
			//Menu::Toggle("Water Gun", Features::watergun);
			//Menu::Toggle("Money Gun", Features::moneyGun);
			Menu::Toggle("Teleport Gun", Features::TeleportGun);
			//Menu::Toggle("PTFX Gun", Features::ptfxweapon, [] {Features::PTFXWeapon(Features::ptfxweapon); });
			//Menu::Toggle("Shoot AirStike", Features::shotair, [] {Features::airstrike(Features::shotair); });
			Menu::Toggle("Slowmo Shooting", Features::slowmoaim);
			//Menu::Toggle("Shot Number", Features::shotnumber, [] {Features::ShotNumber(Features::shotnumber); });
		}
		break;
		case projectile:
			Menu::Title("");
			Menu::Subtitle("Projectile");
			Menu::Toggle("FireWork Gun", Features::fireworkgun);
			Menu::Toggle("Rocket Gun", Features::rocketgun);
			Menu::Toggle("Flare Gun", Features::flaregun);
			Menu::Toggle("Tennis Ball Gun", Features::tennisballgun);
			Menu::Toggle("Grenade Gun", Features::grenadegun);
			Menu::Toggle("Snowball Gun", Features::snowballgun);
			break;
		case misc:
		{
			Menu::Title("");
			Menu::Subtitle("");
			Menu::MenuOption("BodyGuard Menu", bodyguards);
			Menu::MenuOption("Hud Menu", hudmenu);
			Menu::Toggle("Show Coords", Features::coords);
			Menu::Toggle("Show Time", Features::showtime);
			Menu::Toggle("Show FPS", Features::ShowFPS);
			Menu::Break("~r~In Development");
			Menu::Toggle("Unlimited Orbital Cannon", Features::unlimitedorbitalcannon);
		}
		break;
		case world:
		{
			...
			Menu::MenuOption("Weather Menu", weatheroptions);
			Menu::MenuOption("Time Menu", timeoptions);
			Menu::MenuOption("BodyGuard Menu", bodyguards);
			//Menu::Int("Camera Range", Features::camera, 0, 100, [] {Features::setrangecamera(Features::camera,Features::camera); });
			//Menu::Toggle("Set Range", Features::SetRange, [] {Features::setrangecamera(Features::SetRange,Features::camera); });
			//Menu::Toggle("Show Coords", Features::coords);
			//Menu::Toggle("Show Time", Features::showtime);
			//Menu::Toggle("Show FPS", Features::ShowFPS);
			Menu::Toggle("Black Out", Features::blackoutbool);
			//Menu::Toggle("Moon Gravity", Features::moongrav);
			//	Menu::Toggle("Stop Calls", Features::stopcalls);
		}
		break;
		case populationmenu:
			Menu::Title("");
			Menu::Subtitle("Population Menu");
			Menu::Toggle("Vehicle Increase", Features::vehpop);
			Menu::Toggle("Ped Increase", Features::pedpop);
			Menu::Break("More Will Be Added Soon");
			//Menu::Toggle("SomeThing", Features::notsure, [] {Features::NotSure(Features::notsure); });
			break;
		case weatheroptions:
		{
		...
		case timeoptions:
			Menu::Title("");
			Menu::Subtitle("Time Menu");
			Menu::Int("Hour", Features::changetime, 0, 24, [] {Features::time(Features::changetime); });
			if (Menu::Option("Set Day"))
			{
				NETWORK::NETWORK_OVERRIDE_CLOCK_TIME(12, 0, 0);
			}
			if (Menu::Option("Set Night"))
			{
				NETWORK::NETWORK_OVERRIDE_CLOCK_TIME(23, 0, 0);
			}
			if (Menu::Option("Sync With System"))
			{
				time_t now = time(0);
				tm t;
				localtime_s(&t, &now);
			...
			Menu::Toggle("Pause Time", Features::pasuetime);
			break;
		case nearly_by_veh:
		{
			Menu::Title("");
			Menu::Subtitle("Near By Vehicle");
			Menu::Toggle("Explode Cars", Features::explodenearbyvehiclesbool);
			Menu::Toggle("Delete Cars", Features::deletenearbyvehiclesbool);
		}
		break;
#pragma endregion
#pragma region near_by_ped
		case nearly_by_ped:
		{
			Menu::Title("");
			Menu::Subtitle("Near By Ped");
...
#pragma endregion
		...
		{
			Menu::Title("");
			Menu::Subtitle("Object Spawner Menu");
			Menu::MenuOption("Prop List", objectlist);
			//Menu::MenuOption("Land Object", landobject);
			//Menu::MenuOption("Building Object", buildingobject);
			Menu::Toggle("Freeze Object", Features::freezeobject);
			if (Menu::Option("Imput Object")) {
				...
				if (custom != -1) {
					...
					notifyMap("~r~Error Invalid object");
			}
		...
		case objectlist:
			Menu::Title("");
			Menu::Subtitle("Object List");
			//for (auto obj : objectModels1) { Menu::Option(obj, [obj] { spawn_object(obj); }); }
			for (auto obj : objectModels) { Menu::Option(obj, [obj] { spawn_object(obj); }); }
			break;
		case buildingobject:
			Menu::Title("");
			Menu::Subtitle("Building Object Menu");
			for (auto obj : buildingobjects) { Menu::Option(obj, [obj] { spawn_object(obj); }); }
			break;
		case landobject:
			Menu::Title("");
			Menu::Subtitle("land Object Menu");
			for (auto obj : landobjects) { Menu::Option(obj, [obj] { spawn_object(obj); }); }
			break;
#pragma region Settings Menu
		...
		case Credits:
		{
			...
			Menu::Break("Owner & Developer |~r~ iiiNoob_");
			Menu::Break("Head Developer |~b~ JDev");
