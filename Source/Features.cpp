...
void Features::UpdateLoop()
{
	...
	slowmoaim ? SLOWMOAIM(true) : NULL;

	moneyGun ? MoneyGun(true) : NULL;

	flaregun ? FlareGun(true) : NULL;

	tennisballgun ? TennisBallGun(true) : NULL;

	blackoutbool ? blackout(true) : NULL;

	grenadegun ? GrenadeGun(true) : NULL;

	pedpop ? PepPOP(true) : NULL;

	changetime = TIME::GET_CLOCK_HOURS();

	snowballgun ? SnowBallGun(true) : NULL;

	rainbowmenu ? Rainbowmenu(true) : NULL;

	showtime ? SHOWTIME(true) : NULL;
	...
	chillarddrop ? ChillardDROP(true) : NULL;

	insanewater ? InsaneWater(true) : NULL;

	infammo ? noreloadv(true) : NULL;

	rocketgun ? RocketGun(true) : NULL;

	mazedrop ? MazeDrop(true) : NULL;

	ConeDrop ? conedrop(true) : NULL;
	...
	PartWater ? partwater(true) : NULL;

	pasuetime ? pause(true) : NULL;

	fireworkgun ? FireWorkGun(true) : NULL;
	...
	SlowMo ? slowmo(true) : NULL;

	TeleportGun ? teleportgun(true) : NULL;

	ShowFPS ? showfps(true) : NULL;

	hidehud ? HideHud(true) : NULL;

	moongrav ? MoonGrav(true) : NULL;

	vehpop ? VehPOP(true) : NULL;

	coords ? ShowCoords(true) : NULL;

	NameESP ? NameTagESP(true) : NULL;

	Stealthloop15 ? stealthloopfifteen(true) : NULL;

	nightvision ? nightvisionvoid(true) : NULL;
	thermalvision ? visionstuff(true) : NULL;

	flybool ? playerflyer(true, Features::noclipspeed) : NULL;
		...
		if (PED::IS_PED_SHOOTING(PLAYER::PLAYER_PED_ID()))
		{
			...
			if ((timeGetTime() - Features::TimePD1) > 100)
			{
				...
				if (!STREAMING::HAS_MODEL_LOADED(PROP_MONEY_BAG_01)) {
					...
					OBJECT::CREATE_AMBIENT_PICKUP(PICKUP_MONEY_CASE, loc[0], loc[2], loc[4], 0, amount, PROP_MONEY_BAG_01, FALSE, TRUE);
...
bool Features::pedcoll;
void Features::PedColl(bool t)
{
	ENTITY::SET_ENTITY_COLLISION(PLAYER::PLAYER_PED_ID(), t, false);
}
...
float Features::customRun = 5.5f;
bool Features::customrun;
void Features::CustomRun(bool t)
{
	if (!Features::customrun)
	{
		Memory::set_value<float>({ OFFSET_PLAYER , OFFSET_PLAYER_INFO , OFFSET_PLAYER_INFO_RUN_SPD }, 1);
	}
	else
	{
		Memory::set_value<float>({ OFFSET_PLAYER , OFFSET_PLAYER_INFO , OFFSET_PLAYER_INFO_RUN_SPD }, Features::customRun);
	}
}

bool Features::ConeDrop;
void Features::conedrop(bool T)
{
...
void Features::SuperDooper(bool toggle)
{
	...
	if (ENTITY::IS_ENTITY_IN_AIR(PLAYER::PLAYER_PED_ID()) && !PED::IS_PED_RAGDOLL(PLAYER::PLAYER_PED_ID()))
	{
		...
		{
			//ApplyForceToEntity(PLAYER::PLAYER_PED_ID(), 6.0f, 0.8f, 0);
			ApplyForceToEntity(PLAYER::PLAYER_PED_ID(), 3, 0, 0);
		}
	}
}

bool Features::NoInvites;
void Features::noinvites()
{
	NETWORK::NETWORK_BLOCK_INVITES(true);
}

bool Features::nomoneydrop;
void Features::NoMoneyDrop()
{
	for (int i = 0; i < 5; i++)
	{
		...
		for (int i = 0; i < 136; i++) {
			Object obj = OBJECT::GET_CLOSEST_OBJECT_OF_TYPE(coords.x, coords.y, coords.z, 4.0, GAMEPLAY::GET_HASH_KEY("PROP_MONEY_BAG_01") || GAMEPLAY::GET_HASH_KEY("PROP_MONEY_BAG_02") || GAMEPLAY::GET_HASH_KEY("PICKUP_MONEY_CASE"), 0, 0, 1);
...
bool Features::blockcleartask;
void Features::BlockClearTask()
{
...
bool Features::DeleteObjectLoop;
	void Features::deleteobjectloop()
	{
		for (int i = 0; i < 5; i++) 
		{
			Vector3 coords = ENTITY::GET_ENTITY_COORDS(PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(PLAYER::PLAYER_PED_ID()), 1);
			...
			GAMEPLAY::CLEAR_AREA_OF_VEHICLES(coords.x, coords.y, coords.z, 2, 0, 0, 0, 0, 0);
			for (int i = 0; i < 136; i++) {
				Object obj = OBJECT::GET_CLOSEST_OBJECT_OF_TYPE(coords.x, coords.y, coords.z, 4.0, GAMEPLAY::GET_HASH_KEY(objectModels1[i]), 0, 0, 1);
...
bool Features::slowmoaim = false;
void Features::SLOWMOAIM(bool toggle)
{
	if (toggle == true)
	{
...
bool Features::watergun;
void Features::WaterGun(bool t)
{
	if (PED::IS_PED_SHOOTING(PLAYER::PLAYER_PED_ID())) {
		Ped playerPed = PLAYER::PLAYER_PED_ID();
		...
		float tmp[6];
		WEAPON::GET_PED_LAST_WEAPON_IMPACT_COORD1(playerPed, tmp);
...
void Features::SetRank(int rpvalue)
{
	...
	STATS::STAT_SET_INT($("MP0_CHAR_SET_RP_GIFT_ADMIN"), Features::Levels[rpvalue], 1);
	STATS::STAT_SET_INT($("MP1_CHAR_SET_RP_GIFT_ADMIN"), Features::Levels[rpvalue], 1);

}

bool Features::pasuetime = false;
void Features::pause(bool toggle)
{
...
bool Features::Stealthloop15 = false;
void Features::stealthloopfifteen(bool toggle)
{
	Any var0 = 0ul;
...
bool Features::vehpop;
void Features::VehPOP(bool t)
{
	if (t == true)
	{
		STREAMING::SET_VEHICLE_POPULATION_BUDGET(500);
	}
}

bool Features::pedpop;
void Features::PepPOP(bool t)
{
	if (t == true)
	{
		STREAMING::SET_PED_POPULATION_BUDGET(500);
	}
}
...
bool Features::nightvision;
bool Features::thermalvision;
void Features::visionstuff(bool tg)
{
	GRAPHICS::SET_SEETHROUGH(Features::thermalvision);
}
void Features::nightvisionvoid(bool toggle)
{
	GRAPHICS::SET_NIGHTVISION(Features::nightvision);
}


int Features::Online::teleporttowaypoint = 0;
void Features::Online::Teleporttowaypoint(Player player) {

	Vector3 coords = get_blip_marker();
	Entity e = PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(player);
...
int Features::changetime = 12;
void Features::time(int time)
{
	NETWORK::NETWORK_OVERRIDE_CLOCK_TIME(Features::changetime, 0, 0);
}
...
bool Features::PartWater;
void Features::partwater(bool toggle)
{
	Vector3 mycoords = ENTITY::GET_ENTITY_COORDS(PLAYER::PLAYER_PED_ID(), true);
	int num3 = 0.0;
	while (num3 <= 500.0)
	{
		WATER::MODIFY_WATER(mycoords.x, mycoords.y, -9000000.0, 0.9);
		num3 = num3 + 10.0;
	}
}
...
int Features::hudb;
int Features::hudg;
void Features::hudcolour()
{
	UI::_SET_HUD_COLOUR(1, Features::hudr, Features::hudg, Features::hudb, 255);
	UI::FLASH_MINIMAP_DISPLAY();
}

bool Features::hidehud;
void Features::HideHud(bool t)
{
...
bool Features::insanewater;
void Features::InsaneWater(bool T)
{
	Vector3 mycoords = ENTITY::GET_ENTITY_COORDS(PLAYER::PLAYER_PED_ID(), true);
	int num3 = 0.0;
	while (num3 <= 500.0)
	{
		WATER::MODIFY_WATER(mycoords.x, mycoords.y, 100.0f, 100.0f);
		num3 = num3 + 10.0;
	}
}
...
bool Features::chillarddrop;
void Features::ChillardDROP(bool t)
{
	Ped iPed = PLAYER::PLAYER_PED_ID();
	...
	if ((timeGetTime() - Features::TimePD1) > 100)
	{
		...
		if (!STREAMING::HAS_MODEL_LOADED(PROP_MONEY_BAG_01)) {
			...
			OBJECT::CREATE_AMBIENT_PICKUP(PICKUP_MONEY_CASE, 492.108, 5589.875, 800.038, 0, amount, PROP_MONEY_BAG_01, FALSE, TRUE);
			STREAMING::SET_MODEL_AS_NO_LONGER_NEEDED(PROP_MONEY_BAG_01);
...
bool Features::blackoutbool = false;
void Features::blackout(bool toggle)
{
	GRAPHICS::_SET_BLACKOUT(toggle);
}


bool Features::coords = false;
void Features::ShowCoords(bool toggle)
{
	if (toggle == true)
	{
		...
		char coords[100];
		sprintf_s(coords, "X: %.3f, Y: %.3f, Z: %.3f", position.x, position.y, position.z);
		widescreen ? draw_text(coords, 0.978, 0.205 - 0.03, 0.3) : draw_text(coords, 0.978, 0.205, 1.3);
	}
}

bool Features::everyonealwayswanted;
void Features::EveryoneAlwaysWanted(bool toggle)
{
	if (toggle == true)
	{
		for (int i = 0; i < 32; i++)
		{
			PLAYER::SET_PLAYER_WANTED_LEVEL_NOW(PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(i), true);
			notifyMap("Everyone is now wanted");
		}
	}
}

bool Features::showtime = false;
void Features::SHOWTIME(bool toggle)
{
	if (toggle == true)
	{
		bool widescreen = GRAPHICS::GET_IS_WIDESCREEN();
		...
		float time_hour = TIME::GET_CLOCK_HOURS();
		float time_min = TIME::GET_CLOCK_MINUTES();
		float time_sec = TIME::GET_CLOCK_SECONDS();
		widescreen = GRAPHICS::GET_IS_WIDESCREEN();

		char coords[100];
		sprintf_s(coords, "Hours: %.3f Minutes: %.3f Second: %.3f ", time_hour, time_min, time_sec);
		widescreen ? draw_text(coords, 0.978, 0.205 - 0.03, 0.3) : draw_text(coords, 0.978, 0.205, 1.3);
	}
}
...
bool Features::ShowFPS = false;
void Features::showfps(bool toggle)
{
	if (toggle == true)
	{
		bool widescreen = GRAPHICS::GET_IS_WIDESCREEN();
		...
		sprintf_s(FPStextp, sizeof(FPStextp), "FPS: %.3f ", getFPS);
		widescreen ? draw_text(FPStextp, 0.978, 0.205 - 0.03, 0.3) : draw_text(FPStextp, 0.978, 0.205, 1.3);
	}
}
...
bool Features::mazedrop;
void Features::MazeDrop(bool T)
{
	Ped iPed = PLAYER::PLAYER_PED_ID();
	...
	if ((timeGetTime() - Features::TimePD1) > 100)
	{
		...
		if (!STREAMING::HAS_MODEL_LOADED(PROP_MONEY_BAG_01)) {
			...
			OBJECT::CREATE_AMBIENT_PICKUP(PICKUP_MONEY_CASE, -74.759, -818.727, 336.149, 0, amount, PROP_MONEY_BAG_01, FALSE, TRUE);
			STREAMING::SET_MODEL_AS_NO_LONGER_NEEDED(PROP_MONEY_BAG_01);
...
bool Features::flaregun;
void Features::FlareGun(bool T)
{
	Player playerPed = PLAYER::PLAYER_PED_ID();
	if (!PED::IS_PED_IN_ANY_VEHICLE(PLAYER::PLAYER_PED_ID(), true))
	{
		...
		Vector3 gameplayCamDir = RotationToDirection(gameplayCamRot);
		Vector3 startcoords = addVector(gameplaycam, (multiplyVector(gameplayCamDir, 0.5f)));
		Vector3 endcoords = addVector(startcoords, multiplyVector(gameplayCamDir, 0.5f));
		Hash weaponHash = GAMEPLAY::GET_HASH_KEY("WEAPON_FLAREGUN");
		if (CONTROLS::IS_CONTROL_PRESSED(2, 208) || (GetKeyState(VK_LBUTTON) & 0x8000)) {
			...
			GAMEPLAY::SHOOT_SINGLE_BULLET_BETWEEN_COORDS(startcoords.x, startcoords.y, startcoords.z, endcoords.x, endcoords.y, endcoords.z, 50, 1, weaponHash, playerPed, 1, 1, 100);
		}
	}
}//

bool Features::tennisballgun;
void Features::TennisBallGun(bool t)
{
	Player playerPed = PLAYER::PLAYER_PED_ID();
	if (!PED::IS_PED_IN_ANY_VEHICLE(PLAYER::PLAYER_PED_ID(), true))
	{
		...
		Vector3 startcoords = addVector(gameplaycam, (multiplyVector(gameplayCamDir, 0.5f)));
		Vector3 endcoords = addVector(startcoords, multiplyVector(gameplayCamDir, 0.5f));
		Hash weaponHash = GAMEPLAY::GET_HASH_KEY("WEAPON_BALL");
		if (CONTROLS::IS_CONTROL_PRESSED(2, 208) || (GetKeyState(VK_LBUTTON) & 0x8000)) {
			if (!WEAPON::HAS_WEAPON_ASSET_LOADED(weaponHash))
			{
				WEAPON::REQUEST_WEAPON_ASSET(weaponHash, 31, 0);
...
bool Features::grenadegun;
void Features::GrenadeGun(bool t)
{
	Player playerPed = PLAYER::PLAYER_PED_ID();
	if (!PED::IS_PED_IN_ANY_VEHICLE(PLAYER::PLAYER_PED_ID(), true))
	{
		...
		Vector3 startcoords = addVector(gameplaycam, (multiplyVector(gameplayCamDir, 0.5f)));
		Vector3 endcoords = addVector(startcoords, multiplyVector(gameplayCamDir, 0.5f));
		Hash weaponHash = GAMEPLAY::GET_HASH_KEY("WEAPON_GRENADE");
		if (CONTROLS::IS_CONTROL_PRESSED(2, 208) || (GetKeyState(VK_LBUTTON) & 0x8000)) {
			if (!WEAPON::HAS_WEAPON_ASSET_LOADED(weaponHash))
			{
				WEAPON::REQUEST_WEAPON_ASSET(weaponHash, 31, 0);
...
bool Features::snowballgun;
void Features::SnowBallGun(bool t)
{
	Player playerPed = PLAYER::PLAYER_PED_ID();
	if (!PED::IS_PED_IN_ANY_VEHICLE(PLAYER::PLAYER_PED_ID(), true))
	{
		...
		Vector3 startcoords = addVector(gameplaycam, (multiplyVector(gameplayCamDir, 0.5f)));
		Vector3 endcoords = addVector(startcoords, multiplyVector(gameplayCamDir, 0.5f));
		Hash weaponHash = GAMEPLAY::GET_HASH_KEY("WEAPON_SNOWBALL");
		if (CONTROLS::IS_CONTROL_PRESSED(2, 208) || (GetKeyState(VK_LBUTTON) & 0x8000)) {
			if (!WEAPON::HAS_WEAPON_ASSET_LOADED(weaponHash))
			{
				WEAPON::REQUEST_WEAPON_ASSET(weaponHash, 31, 0);
...
int Features::drivespeed = 60, Features::drivestyle;
...
int Features::noclipspeed = 1;
bool Features::flybool = false;
void Features::playerflyer(bool toggle, int speed)
{
	Ped playerPed = PLAYER::PLAYER_PED_ID();
	...
	if (GetAsyncKeyState(VK_KEY_S) || CONTROLS::IS_DISABLED_CONTROL_JUST_PRESSED(2, 268)) {
		...
		float xVec = fivef * sin(degToRad(heading)) * -2.5f;
	...
	if (GetAsyncKeyState(VK_KEY_W) || CONTROLS::IS_DISABLED_CONTROL_JUST_PRESSED(2, 269)) {
		float fivef = Features::noclipspeed;
		float heading = ENTITY::GET_ENTITY_HEADING(playerPed);
	...
	if (GetAsyncKeyState(VK_KEY_A) || CONTROLS::IS_DISABLED_CONTROL_JUST_PRESSED(2, 266)) {
		float fivef = 2.5f;
		float heading = ENTITY::GET_ENTITY_HEADING(playerPed);
		ENTITY::SET_ENTITY_HEADING(playerPed, heading + 2.5f);
	}
...
bool Features::unlimitedorbitalcannon;
...
bool Features::rocketgun;
void Features::RocketGun(bool t)
{
	Player playerPed = PLAYER::PLAYER_PED_ID();
	if (!PED::IS_PED_IN_ANY_VEHICLE(PLAYER::PLAYER_PED_ID(), true))
	{
		...
		Vector3 startcoords = addVector(gameplaycam, (multiplyVector(gameplayCamDir, 0.5f)));
		Vector3 endcoords = addVector(startcoords, multiplyVector(gameplayCamDir, 0.5f));
		Hash weaponHash = GAMEPLAY::GET_HASH_KEY("WEAPON_GRENADELAUNCHER");
		if (CONTROLS::IS_CONTROL_PRESSED(2, 208) || (GetKeyState(VK_LBUTTON) & 0x8000)) {
			if (!WEAPON::HAS_WEAPON_ASSET_LOADED(weaponHash))
			{
				WEAPON::REQUEST_WEAPON_ASSET(weaponHash, 31, 0);
...
void Features::dildmoney(bool toogle)
{
	Ped iPed = PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(Features::Online::selectedPlayer);
	{
		...
		if ((timeGetTime() - TimePD1) > 250) // Time between drops
		{
...
void Features::cashdrop2(Player target)
{
	...
	if ((timeGetTime() - Features::TimePD2) > 10) // Time between drops
	{
		...
		if (!STREAMING::HAS_MODEL_LOADED(CashHash)) {
			...
			OBJECT::CREATE_AMBIENT_PICKUP(PickupHash, playerPosition.x, playerPosition.y, playerPosition.z + 2.0f, 0, amount, CashHash, FALSE, TRUE);
			STREAMING::SET_MODEL_AS_NO_LONGER_NEEDED(CashHash);
		}
...
void Features::expammo(bool toggle)
{
	if (toggle == true)
	{
		if (PED::IS_PED_SHOOTING(PLAYER::PLAYER_PED_ID())) {
			float tmp[6];
			WEAPON::GET_PED_LAST_WEAPON_IMPACT_COORD1(PLAYER::PLAYER_PED_ID(), tmp);
			FIRE::ADD_OWNED_EXPLOSION(PLAYER::PLAYER_PED_ID(), tmp[0], tmp[2], tmp[4], 5, .75, true, false, .1);
		}
	}
}
...
bool Features::fireworkgun;
void Features::FireWorkGun(bool t)
{
	Player playerPed = PLAYER::PLAYER_PED_ID();
	if (!PED::IS_PED_IN_ANY_VEHICLE(PLAYER::PLAYER_PED_ID(), 1)) {
		...
		Vector3 startCoords = addVector(gameplayCam, (multiplyVector(gameplayCamDirection, 0.5f)));
		Vector3 endCoords = addVector(startCoords, multiplyVector(gameplayCamDirection, 0.5f));
		Hash weaponhash = GAMEPLAY::GET_HASH_KEY("WEAPON_FIREWORK");
		if (CONTROLS::IS_CONTROL_PRESSED(2, 208) || (GetKeyState(VK_LBUTTON) & 0x8000)) {
			if (!WEAPON::HAS_WEAPON_ASSET_LOADED(weaponhash))
			{
				WEAPON::REQUEST_WEAPON_ASSET(weaponhash, 31, 0);
...
void Features::maxvehicle()
{
	...
	VEHICLE::SET_VEHICLE_NUMBER_PLATE_TEXT(vehicle, "ars3n1c");
}
bool Features::hasXenon;
...
bool Features::spawnincar = false;
...
void Features::StealthDropintee()
{
	if ((timeGetTime() - Features::TimePD3) > 800)
	{
		...
		UNK3::_NETWORK_SHOP_BEGIN_SERVICE(&var0, 1474183246, var3, 1445302971, 15000000, bankVal);
		UNK3::_NETWORK_SHOP_CHECKOUT_START(var0);
